...
void Scheduler::InitTimes()
{
  ...
  timeThread.Init("Threads", threadGroup.size());
...
void Scheduler::Reset()
{
  ...
  std::fill(std::begin(threadGroup), std::end(threadGroup), -1);
  std::fill(std::begin(threadCurrGroup), std::end(threadCurrGroup), -1);
